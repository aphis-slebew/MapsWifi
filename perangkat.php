...
// PERBAIKAN: Panggil koneksi tanpa ../ karena file ini ada di luar folder
...
$where = "";
if ($search != '') {
    $where = " WHERE nama LIKE '%$search%' OR jenis LIKE '%$search%' ";
}

$query = mysqli_query($koneksi, "SELECT * FROM perangkat $where ORDER BY id DESC");
    ...
    <main style="flex: 1; padding: 24px; overflow-y: auto;">
        ...
        <div class="table-card" style="background: var(--card-bg); padding: 24px; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); border: 1px solid var(--border-color);">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;">
                <h3 style="margin: 0;">Data Perangkat Server/ODP</h3>
                
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <form method="GET" action="" style="display: flex; gap: 10px;">
                        <input type="text" name="search" value="<?= htmlspecialchars($search) ?>" placeholder="Cari Nama / Jenis..." style="padding: 10px 15px; border-radius: 8px; border: 1px solid var(--border-color); outline: none; width: 220px; background: var(--input-bg); color: var(--text-main);">
                        <button type="submit" class="btn-primary" style="width: auto; padding: 10px 15px; cursor: pointer;"><i class='bx bx-search'></i></button>
                        <?php if($search): ?>
                            <a href="perangkat.php" class="btn-primary" style="width: auto; padding: 10px 15px; background: #ef4444; text-decoration: none; display: flex; align-items: center;"><i class='bx bx-x'></i></a>
                        <?php endif; ?>
                    </form>
                ...
                <table style="width: 100%; border-collapse: collapse; text-align: left;">
                    ...
                    <tbody>
                        ...
                        <tr style="border-bottom: 1px solid var(--border-color);">
                            ...
                            <td style="padding: 12px; text-transform: uppercase;"><?= $row['jenis'] ?></td>
                            <td style="padding: 12px;">
                                <?php if($row['lat']): ?>
                            ...
                            <td style="padding: 12px; display: flex; gap: 5px; flex-wrap: wrap;">
                                <a href="index.php?id_target=<?= $row['id'] ?>&tipe=perangkat" title="Plot ke Peta" style="background: #3b82f6; color: white; padding: 6px 10px; border-radius: 6px; text-decoration: none;"><i class='bx bx-map-pin'></i></a>
                                <a href="edit_perangkat.php?id=<?= $row['id'] ?>" title="Edit Data" style="background: #eab308; color: white; padding: 6px 10px; border-radius: 6px; text-decoration: none;"><i class='bx bx-edit'></i></a>
                                <a href="api/hapus_data.php?id=<?= $row['id'] ?>&tipe=perangkat" onclick="return confirm('Hapus permanen?')" title="Hapus Data" style="background: #ef4444; color: white; padding: 6px 10px; border-radius: 6px; text-decoration: none;"><i class='bx bx-trash'></i></a>
                            </td>
                        </tr>
                        <?php 
                            endwhile; 
...
<div id="modalTambah" class="modal-overlay">
    <div class="modal-content">
        ...
        <h3 style="margin-bottom: 20px; color: var(--text-main);">Tambah Perangkat</h3>
        
        <form action="api/tambah_perangkat.php" method="POST" enctype="multipart/form-data">
            <div class="input-group">
                <label>Nama Perangkat</label>
                <input type="text" name="nama" placeholder="Contoh: ODP 01..." required>
            </div>
            <div class="input-group">
